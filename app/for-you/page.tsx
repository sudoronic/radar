...
export default async function ForYou({searchParams}:{searchParams:Promise<{page?:string;tab?:string}>}){
 const {profile,preferences}=await viewer();const params=await searchParams;const page=Math.max(0,Math.min(10,Number(params.page)||0));
